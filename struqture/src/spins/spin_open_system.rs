...
impl SpinLindbladOpenSystem {
    ...
    pub fn to_struqture_1(
        ...
        struqture_one::OpenSystem::group(new_system, new_noise).map_err(
            |err| StruqtureError::GenericError { msg:
                format!("Could not convert struqture 2.x SpinLindbladOpenSystem to 1.x SpinLindbladOpenSystem, group function failed: {:?}.", err)
            }
        )
    }
    ...
    pub fn from_struqture_1(
        ...
        let new_system = SpinHamiltonian::from_struqture_1(&system_one)?;
        let new_noise = SpinLindbladNoiseOperator::from_struqture_1(&noise_one)?;
