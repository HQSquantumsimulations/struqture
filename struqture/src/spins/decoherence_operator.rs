...
use crate::{
    OperateOnDensityMatrix, OperateOnState, SpinIndex, StruqtureError, StruqtureVersion,
    SymmetricIndex,
};
...
mod test {
    ...
    fn debug() {
        ...
        assert_eq!(
            format!("{:?}", sos),
            "DecoherenceOperatorSerialize { items: [(DecoherenceProduct { items: [(0, Z)] }, Float(0.5), Float(0.0))], _struqture_version: StruqtureVersion }"
        );
    }
