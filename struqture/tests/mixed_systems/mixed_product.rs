...
use struqture::{CorrespondsTo, StruqtureError};
...
fn new_normal_ordered_passing(
    ...
    let test_new = MixedProduct::new([spins.clone()], [bosons.clone()], [fermions.clone()]);
    ...
    assert_eq!(res.corresponds_to(), res);

    let (valid_pair, coeff) = MixedProduct::create_valid_pair(
        [spins.clone()],
        [bosons.clone()],
        [fermions.clone()],
        1.0.into(),
...
fn from_string_import_export(
    ...
    let test_new = <MixedProduct as std::str::FromStr>::from_str(&stringformat).unwrap();
...
fn serialize_bincode(
    ...
    let fermions =
        FermionProduct::new(fermion_creators.to_vec(), fermion_annihilators.to_vec()).unwrap();
    let test_new = MixedProduct::new([spins], [bosons], [fermions]);
    assert!(test_new.is_ok());
    let res = test_new.unwrap();
    let serialized = serialize(&res).unwrap();
    let deserialized: MixedProduct = deserialize(&serialized).unwrap();
...
fn serialize_json(
    ...
    let deserialized: MixedProduct = serde_json::from_str(&serialized).unwrap();
    assert_eq!(res, deserialized);
}
...
fn multiply_spins() {
    ...
    let comparison: Vec<(MixedProduct, Complex64)> = vec![(
        MixedProduct::new([PauliProduct::new().z(0).x(1)], vec![], vec![]).unwrap(),
        (1.0).into(),
    )];
...
fn multiply_spins_bosons() {
    ...
    .unwrap();
    let right = MixedProduct::new([spins_right], [bosons_right, bosons_left], []).unwrap();

    let result = (left * right).unwrap();
    let comparison: Vec<(MixedProduct, Complex64)> = vec![
        (
            MixedProduct::new(
                [PauliProduct::new().z(0).x(1)],
                vec![
...
fn multiply_spins_fermions() {
    ...
    .unwrap();
    let right = MixedProduct::new([spins_right], [], [fermions_right, fermions_left]).unwrap();
...
fn mixed_default(spins: &[PauliProduct], bosons: &[BosonProduct], fermions: &[FermionProduct]) {
    let test_new = MixedProduct::default();
...
fn hermitian_test() {
    ...
    let test_new = MixedProduct::new([spins.clone()], [bosons], [fermions]).unwrap();
    ...
    let hermitian_test = MixedProduct::new([spins], [bosons_h], [fermions_h]).unwrap();
...
fn get_value_mixed() {
    ...
    let bosons = BosonProduct::new(creators.to_vec(), annihilators.to_vec()).unwrap();
    let fermions = FermionProduct::new(creators.to_vec(), annihilators.to_vec()).unwrap();
    let test_new = MixedProduct::new([spins], [bosons], [fermions]).unwrap();

    assert_eq!(MixedProduct::get_key(&test_new), test_new);
    assert_eq!(
        MixedProduct::get_transform(&test_new, CalculatorComplex::new(1.0, 2.0)),
...
fn debug() {
    ...
    assert_eq!(
        format!("{:?}", test_new),
        "MixedProduct { spins: [PauliProduct { items: [(0, X), (1, Y), (2, Z)] }], bosons: [BosonProduct { creators: [0, 1, 1], annihilators: [3, 3, 5] }], fermions: [FermionProduct { creators: [0, 1, 2], annihilators: [3, 4, 5] }] }"
    );
    assert_eq!(
        format!("{}", test_new),
...
fn hash_debug() {
    ...
    let fermions = FermionProduct::new(f_creators.to_vec(), f_annihilators.to_vec()).unwrap();
    let test_new =
        MixedProduct::new([spins.clone()], [bosons.clone()], [fermions.clone()]).unwrap();
    let test_new_1 = MixedProduct::new([spins], [bosons], [fermions]).unwrap();
...
fn clone_partial_eq_partial_ord() {
    ...
    let test_0 = MixedProduct::new([spins.clone()], [bosons.clone()], [fermions.clone()]).unwrap();
    let test_1 = MixedProduct::new(
    ...
    let test_0 = MixedProduct::new([spins], [bosons.clone()], [fermions.clone()]).unwrap();
    let test_1 =
        MixedProduct::new([PauliProduct::from_str("").unwrap()], [bosons], [fermions]).unwrap();
...
fn test_mixed_product_schema() {
    ...
    .unwrap();
    let schema = schemars::schema_for!(MixedProduct);
