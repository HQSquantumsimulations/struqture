...
use struqture::mixed_systems::{MixedOperator, MixedProduct, MixedSystem};
...
#[test]
fn new_empty() {
    let mo = MixedSystem::new([], [], []);
    assert_eq!(mo.operator(), &MixedOperator::default());
}
...
fn new_current(
    ...
    let mo = MixedSystem::new([Some(n_pauli)], [Some(n_bosons)], [Some(n_fermions)]);
    assert!(mo.is_empty());
...
fn from_spin_operator_none() {
    let mut mo: MixedOperator = MixedOperator::new(1, 1, 1);
    let mut system = MixedSystem::new([None], [None], [None]);
    ...
    assert_eq!(
        system,
        MixedSystem::from_operator(mo.clone(), [None], [None], [None]).unwrap()
    );
    assert_eq!(
        system.operator(),
        MixedSystem::from_operator(mo.clone(), [None], [None], [None])
            .unwrap()
            .operator()
    );
    assert_eq!(
...
fn from_spin_operator_some() {
    let mut mo: MixedOperator = MixedOperator::new(1, 1, 1);
    let mut system = MixedSystem::new([Some(2)], [Some(2)], [Some(2)]);
    let pp: MixedProduct = MixedProduct::new(
        [PauliProduct::new().x(0)],
        [BosonProduct::new([0], [1]).unwrap()],
    ...
    assert_eq!(
        system,
        MixedSystem::from_operator(mo.clone(), [Some(2)], [Some(2)], [Some(2)]).unwrap()
    );
    assert_eq!(
        system.operator(),
        MixedSystem::from_operator(mo.clone(), [Some(2)], [Some(2)], [Some(2)])
            .unwrap()
            .operator()
    );
    assert_eq!(
    ...
    assert_eq!(
        MixedSystem::from_operator(mo.clone(), [Some(2)], [Some(2)], [Some(1)]),
        Err(StruqtureError::NumberSpinsExceeded {})
    );
}

// Test the current_number_spins function of the MixedSystem
#[test]
fn internal_map_current_number_spins_and_modes() {
    ...
    assert_eq!(mo.number_spins(), vec![3_usize]);
    assert_eq!(mo.current_number_bosonic_modes(), vec![0_usize]);
    assert_eq!(mo.number_bosonic_modes(), vec![4_usize]);
    assert_eq!(mo.current_number_fermionic_modes(), vec![0_usize]);
    assert_eq!(mo.number_fermionic_modes(), vec![4_usize]);
...
// Test the current_number_spins function of the MixedSystem
#[test]
fn internal_map_current_number_spins_and_modes_none() {
    let pp_0: MixedProduct = MixedProduct::new(
        [PauliProduct::new().x(0)],
        [BosonProduct::new([0], [1]).unwrap()],
        [FermionProduct::new([0], [2]).unwrap()],
    ...
    .unwrap();
    let mut mo = MixedSystem::new([None], [None], [None]);
    assert_eq!(mo.current_number_spins(), vec![0_usize]);
    ...
    mo.set(pp_0, CalculatorComplex::from(0.5)).unwrap();
    assert_eq!(mo.current_number_spins(), vec![1_usize]);
    assert_eq!(mo.number_spins(), vec![1_usize]);
    assert_eq!(mo.current_number_bosonic_modes(), vec![2_usize]);
    assert_eq!(mo.number_bosonic_modes(), vec![2_usize]);
    assert_eq!(mo.current_number_fermionic_modes(), vec![3_usize]);
    assert_eq!(mo.number_fermionic_modes(), vec![3_usize]);
...
// Test the iter, keys and values functions of the MixedSystem
#[test]
...
// Test the negative operation: -MixedSystem
#[test]
fn negative_mo() {
    let pp_0: MixedProduct = MixedProduct::new(
        [PauliProduct::new().z(2)],
...
fn mul_so_so_error() {
    ...
    mo_0.add_operator_product(pp_0, CalculatorComplex::from(1.0))
        .unwrap();
    let mut mo_1 = MixedSystem::new([Some(2)], [], [Some(4)]);
...
fn into_iter_from_iter_extend() {
    assert_eq!(
        MixedSystem::from_iter(MixedSystem::default().into_iter()),
        MixedSystem::new([], [], [])
    );
    let pp_0: MixedProduct = MixedProduct::new(
        [PauliProduct::new().z(2)],
    ...
    .unwrap();
    let mut mo_0 = MixedSystem::new([None], [None], [None]);
    mo_0.add_operator_product(pp_0.clone(), CalculatorComplex::from(2.0))
        .unwrap();
    ...
    assert_eq!(MixedSystem::from_iter(mo_iter), mo_0);
    let mo_iter = (&mo_0)
        .into_iter()
    ...
    let mut mo_0_1 = MixedSystem::new([None], [None], [None]);
...
fn from_iterator() {
    ...
    assert_eq!(
        MixedSystem::from_iter(iterator.iter().map(|(x, y)| (x.clone(), y.clone()))),
        mo_0
    );

    // iterator with no items
    let mo_0 = MixedSystem::new([], [], []);
...
#[test]
fn default() {
    assert_eq!(MixedSystem::default(), MixedSystem::new([], [], []));
}
...
fn serde_json() {
    ...
    let deserialized: MixedSystem = serde_json::from_str(&serialized).unwrap();
...
#[test_case(None)]
#[test_case(Some(4))]
fn test_mixed_system_schema(number_particles: Option<usize>) {
    let mut op = MixedSystem::new(
