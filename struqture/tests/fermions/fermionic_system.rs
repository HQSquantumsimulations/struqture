...
fn separate_out_terms(number_spins: (usize, usize)) {
    ...
    let mut separated = FermionSystem::default();
    for (key, value) in allowed.iter() {
        separated
            .add_operator_product(key.clone(), value.into())
            .unwrap();
    }
    let mut remainder = FermionSystem::default();
    ...
    let mut so = FermionSystem::default();
...
fn bincode() {
    ...
    let (deserialized, _len): (FermionSystem, usize) =
...
#[test_case(None)]
#[test_case(Some(3))]
fn test_fermion_system_schema(number_fermions: Option<usize>) {
    let mut op = FermionSystem::new(number_fermions);
    op.set(FermionProduct::new([0], [0]).unwrap(), 1.0.into())
        .unwrap();
    op.set(FermionProduct::new([1], [1]).unwrap(), "val".into())
        .unwrap();
    let schema = schemars::schema_for!(FermionSystem);
