...
fn boson_system_test_set_get() {
    pyo3::prepare_freethreaded_python();
    pyo3::Python::with_gil(|py| {
        let new_system = py.get_type_bound::<BosonOperatorWrapper>();
...
fn test_from_pyany_to_struqture_1() {
    ...
    pyo3::Python::with_gil(|py| {
        ...
        sys_2
            .call_method1("add_operator_product", ("c0c1a0a1", 0.1))
            .unwrap();
        let mut sys_1 = struqture_1::bosons::BosonSystem::new(None);
        struqture_1::OperateOnDensityMatrix::set(
            &mut sys_1,
            struqture_1::bosons::BosonProduct::from_str("c0c1a0a1").unwrap(),
            0.1.into(),
        )
        .unwrap();

        let result = BosonOperatorWrapper::from_pyany_to_struqture_1(sys_2.as_ref()).unwrap();
