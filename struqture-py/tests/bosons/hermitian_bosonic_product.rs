...
) -> Bound<HermitianBosonProductWrapper> {
    let pp_type = py.get_type::<HermitianBosonProductWrapper>();
    pp_type
        .call1((creators, annihilators))
        .unwrap()
        .downcast::<HermitianBosonProductWrapper>()
        .unwrap()
        .to_owned()
}
...
/// Test default function of HermitianBosonProductWrapper
#[test]
fn test_default_partialeq_debug_clone() {
    pyo3::prepare_freethreaded_python();
    Python::with_gil(|py| {
        let pp = new_pp(py, vec![0, 1], vec![1, 2]);
        let pp_wrapper = pp.extract::<HermitianBosonProductWrapper>().unwrap();

        // PartialEq
        let helper_ne: bool = HermitianBosonProductWrapper::default() != pp_wrapper;
        assert!(helper_ne);
        let helper_eq: bool = HermitianBosonProductWrapper::default()
            == HermitianBosonProductWrapper::new(vec![], vec![]).unwrap();
        assert!(helper_eq);

        // Test PartialOrd trait
        let pp_0 = new_pp(py, vec![0, 1], vec![1, 2]);
        let pp_wrapper_0 = pp_0.extract::<HermitianBosonProductWrapper>().unwrap();
        let pp_1 = new_pp(py, vec![0, 1], vec![1, 3]);
        let pp_wrapper_1 = pp_1.extract::<HermitianBosonProductWrapper>().unwrap();
        ...
        assert_eq!(
            format!("{:?}", HermitianBosonProductWrapper::new(vec![0, 1], vec![1, 2]).unwrap()),
            "HermitianBosonProductWrapper { internal: HermitianBosonProduct { creators: [0, 1], annihilators: [1, 2] } }"
        );
    })
}
...
fn test_new_error() {
    ...
    pyo3::Python::with_gil(|py| {
        let pp_type = py.get_type::<HermitianBosonProductWrapper>();
        let pp = pp_type.call1(([2_u64, 3_u64], [1_u64, 1_u64]));
        assert!(pp.is_err());
    });
}

/// Test from_string function of HermitianBosonProduct
#[test]
fn test_from_string() {
    pyo3::prepare_freethreaded_python();
    pyo3::Python::with_gil(|py| {
...
/// Test from_string function of HermitianBosonProduct - PyValueError
#[test]
...
/// Test creators and annihilators functions of HermitianBosonProduct
#[test]
...
/// Test remap_modes function of HermitianBosonProduct
#[test]
fn test_remap_modes() {
    pyo3::prepare_freethreaded_python();
    pyo3::Python::with_gil(|py| {
        let hbp = new_pp(py, vec![], vec![0, 1]);
        let remapped_hbp = new_pp(py, vec![], vec![2, 3]);
        let expected_coeff = CalculatorComplexWrapper {
            internal: 1.0.into(),
        };
        let remap_dict = [(0, 3), (1, 2), (2, 0), (3, 1)].into_py_dict(py).unwrap();
        let results = hbp.call_method1("remap_modes", (remap_dict,)).unwrap();
        let comparison = bool::extract_bound(
            &results
                .call_method1("__eq__", ((remapped_hbp, expected_coeff),))
                .unwrap(),
        )
        .unwrap();
        assert!(comparison);
...
/// Test hermitian_conjugate and is_natural_hermitian functions of HermitianBosonProduct
#[test]
...
fn test_multiply() {
    pyo3::prepare_freethreaded_python();
    pyo3::Python::with_gil(|py| {
        let pp_0 = new_pp(py, vec![0, 0], vec![1, 20]);
        let pp_1 = new_pp(py, vec![1, 30], vec![84, 95]);
        let pp_mul_1 = new_pp_bosonic(py, vec![0, 0, 30], vec![20, 84, 95]);
        let pp_mul_2 = new_pp_bosonic(py, vec![0, 0, 1, 30], vec![1, 20, 84, 95]);
        let pp_mul_3 = new_pp_bosonic(py, vec![0, 0, 84, 95], vec![1, 1, 20, 30]);
        let pp_mul_4 = new_pp_bosonic(py, vec![1, 1, 20, 30], vec![0, 0, 84, 95]);
        let pp_mul_5 = new_pp_bosonic(py, vec![1, 20, 84, 95], vec![0, 0, 1, 30]);
        ...
        let comparison = bool::extract_bound(
            &multiplied
                .call_method1(
                    "__eq__",
                    (vec![pp_mul_1, pp_mul_2, pp_mul_3, pp_mul_4, pp_mul_5],),
                )
                .unwrap(),
        )
        .unwrap();
...
/// Test copy and deepcopy functions of HermitianBosonProduct
#[test]
...
/// Test to_bincode and from_bincode functions of HermitianBosonProduct
#[test]
...
/// Test to_ and from_json functions of HermitianBosonProduct
#[test]
...
/// Test hash functions of HermitianBosonProduct
#[test]
fn test_hash() {
    pyo3::prepare_freethreaded_python();
    pyo3::Python::with_gil(|py| {
...
fn test_json_schema() {
    ...
    pyo3::Python::with_gil(|py| {
        ...
        let rust_schema =
            serde_json::to_string_pretty(&schemars::schema_for!(HermitianBosonProduct)).unwrap();
...
fn test_from_pyany_to_struqture_1() {
    ...
    pyo3::Python::with_gil(|py| {
        ...
        let result =
            HermitianBosonProductWrapper::from_pyany_to_struqture_1(pp_2.as_ref().into()).unwrap();
        assert_eq!(
            result,
