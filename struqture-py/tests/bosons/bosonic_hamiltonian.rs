...
fn test_from_pyany_to_struqture_one() {
    ...
    pyo3::Python::with_gil(|py| {
        ...
        sys_2
            .call_method1("add_operator_product", ("c0c1a0a1", 0.1))
            .unwrap();
        let mut sys_1 = struqture_one::bosons::BosonHamiltonianSystem::new(None);
        struqture_one::OperateOnDensityMatrix::set(
            &mut sys_1,
            struqture_one::bosons::HermitianBosonProduct::from_str("c0c1a0a1").unwrap(),
            0.1.into(),
        )
        .unwrap();

        let result =
            BosonHamiltonianWrapper::from_pyany_to_struqture_one(sys_2.as_ref().into()).unwrap();
