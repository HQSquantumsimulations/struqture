...
fn boson_system_test_set_get() {
    ...
    pyo3::Python::attach(|py| {
        ...
        let system = binding.cast::<BosonHamiltonianSystemWrapper>().unwrap();
...
fn test_copy_deepcopy() {
    ...
    pyo3::Python::attach(|py| {
        ...
        let comparison_deepcopy =
            bool::extract(deepcopy_system.call_method1("__eq__", (system,)).unwrap().as_borrowed().as_borrowed())
                .unwrap();
...
fn test_richcmp() {
    ...
    pyo3::Python::attach(|py| {
        ...
        let comparison =
            bool::extract(system_one.call_method1("__eq__", (&system_two,)).unwrap().as_borrowed().as_borrowed())
                .unwrap();
        assert!(!comparison);
        let comparison =
            bool::extract(system_one.call_method1("__eq__", ("c0c1a0a1",)).unwrap().as_borrowed().as_borrowed())
                .unwrap();
        assert!(!comparison);

        let comparison =
            bool::extract(system_one.call_method1("__ne__", (system_two,)).unwrap().as_borrowed().as_borrowed())
                .unwrap();
        assert!(comparison);
        let comparison =
            bool::extract(system_one.call_method1("__ne__", ("c0c1a0a1",)).unwrap().as_borrowed().as_borrowed())
                .unwrap();
        assert!(comparison);
