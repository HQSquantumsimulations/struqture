...
fn new_pp(
    ...
    pp_type
        ...
        .unwrap()
        .cast::<HermitianFermionProductWrapper>()
        .unwrap()
        .to_owned()
}
...
fn test_jordan_wigner() {
    ...
    pyo3::Python::attach(|py| {
        ...
        let empty = bool::extract(sh.call_method0("is_empty").unwrap().as_borrowed()).unwrap();
        assert!(!empty);

        let current_number_modes = usize::extract(
            hfp.call_method0("current_number_modes")
                .unwrap()
                .as_borrowed(),
        )
        .unwrap();
        let current_number_spins = usize::extract(
            sh.call_method0("current_number_spins")
                .unwrap()
                .as_borrowed(),
        )
        .unwrap();
