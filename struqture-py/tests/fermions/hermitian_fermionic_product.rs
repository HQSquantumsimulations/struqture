...
fn test_jordan_wigner() {
    ...
    pyo3::Python::attach(|py| {
        ...
        let number_modes =
            usize::extract(hfp.call_method0("current_number_modes").unwrap().as_borrowed()).unwrap();
        let number_spins =
            usize::extract(sh.call_method0("current_number_spins").unwrap().as_borrowed()).unwrap();
