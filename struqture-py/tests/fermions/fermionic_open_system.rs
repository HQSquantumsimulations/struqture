...
use struqture_py::fermions::{
    FermionHamiltonianWrapper, FermionLindbladNoiseOperatorWrapper,
...
fn test_default_partialeq_debug_clone() {
    ...
    Python::with_gil(|py| {
        ...
        assert!(helper_ne);
        let helper_eq: bool =
            FermionLindbladOpenSystemWrapper::default() == FermionLindbladOpenSystemWrapper::new();
        assert!(helper_eq);
        ...
        let system_type = py.get_type_bound::<FermionHamiltonianWrapper>();
        let fermion_system = system_type.call0().unwrap();
        fermion_system
        ...
        fermion_system
            ...
            .unwrap();
        let comparison =
            bool::extract_bound(&comp_op.call_method1("__eq__", (fermion_system,)).unwrap())
                .unwrap();
        assert!(comparison);

        // Noise
        let comp_op = new_sys.call_method0("noise").unwrap();
        let noise_type = py.get_type_bound::<FermionLindbladNoiseOperatorWrapper>();
        ...
        let comparison = bool::extract_bound(
            &comp_op_ungroup
                .call_method1("__eq__", ((fermion_system.clone(), noise.clone()),))
                .unwrap(),
        )
        .unwrap();
        assert!(comparison);
        let comp_op_group = new_system(py)
            .call_method1("group", (fermion_system, noise))
            .unwrap();
        let comparison =
...
fn test_from_pyany_to_struqture_1() {
    ...
    pyo3::Python::with_gil(|py| {
        ...
        let pp_1 = struqture_1::fermions::HermitianFermionProduct::from_str("c0a0").unwrap();
        let dp_1 = struqture_1::fermions::FermionProduct::from_str("c1a1").unwrap();
        let mut sys_1 = struqture_1::fermions::FermionLindbladOpenSystem::new(None);
        ...
        let result =
            FermionLindbladOpenSystemWrapper::from_pyany_to_struqture_1(sys_2.as_ref()).unwrap();
