...
use struqture_py::fermions::{
    ...
    FermionLindbladOpenSystemWrapper,
};
...
fn new_system(py: Python) -> Bound<FermionLindbladOpenSystemWrapper> {
    ...
    system_type
        .call0()
        .unwrap()
        .cast::<FermionLindbladOpenSystemWrapper>()
        .unwrap()
        .to_owned()
}
...
/// Test default function of FermionLindbladOpenSystemWrapper
#[test]
fn test_default_partialeq_debug_clone() {
    ...
    Python::attach(|py| {
        ...
        new_sys = new_sys
            ...
            .unwrap();
        let system_wrapper = new_sys
            .extract::<FermionLindbladOpenSystemWrapper>()
            .unwrap();

        // PartialEq
        let helper_ne: bool = FermionLindbladOpenSystemWrapper::default() != system_wrapper;
        assert!(helper_ne);
        let helper_eq: bool = FermionLindbladOpenSystemWrapper::default()
            == FermionLindbladOpenSystemWrapper::new(None);
        assert!(helper_eq);
        ...
        // Debug
        assert_eq!(
            format!("{:?}", FermionLindbladOpenSystemWrapper::new(None)),
            "FermionLindbladOpenSystemWrapper { internal: FermionLindbladOpenSystem { system: FermionHamiltonianSystem { number_modes: None, hamiltonian: FermionHamiltonian { internal_map: {} } }, noise: FermionLindbladNoiseSystem { number_modes: None, operator: FermionLindbladNoiseOperator { internal_map: {} } } } }"
        );
        ...
        fermion_system
            ...
            .unwrap();
        let comparison =
            bool::extract(comp_op.call_method1("__eq__", (fermion_system,)).unwrap().as_borrowed())
                .unwrap();
        assert!(comparison);
        ...
        noise
            .cast::<FermionLindbladNoiseSystemWrapper>()
            .unwrap()
            .call_method1(
                "add_operator_product",
                (
            ...
            .unwrap();
        let comparison =
            bool::extract(comp_op.call_method1("__eq__", (noise,)).unwrap().as_borrowed()).unwrap();
        assert!(comparison);
        ...
        let comp_op_group = new_system(py)
            ...
            .unwrap();
        let comparison =
            bool::extract(comp_op_group.call_method1("__eq__", (new_sys,)).unwrap().as_borrowed())
                .unwrap();
        assert!(comparison);
    })
}

/// Test set_pauli_product and get_pauli_product functions of FermionLindbladOpenSystem
#[test]
...
/// Test set_noise and get_noise functions of FermionLindbladOpenSystem
#[test]
fn test_set_noise_get_noise() {
    ...
    pyo3::Python::attach(|py| {
        ...
        // Get error 1a: Key left (FermionProduct) cannot be converted from string
        let error = system.call_method1("noise_get", (("1+c0a1", "c1a2"),));
        assert!(error.is_err()); // same error as in ADP - Ask!

        // Get error 1b: Key right (FermionProduct) cannot be converted from string
        let error = system.call_method1("noise_get", (("c1a2", "1+c0a1"),));
        assert!(error.is_err()); // same error as in ADP - Ask!

        // Try_set error 1a: Key left (FermionProduct) cannot be converted from string
        let error = system.call_method1(
            "set",
            (
                ("1+c0a1", "c1a2"),
        ...
        // Try_set error 1b: Key right (FermionProduct) cannot be converted from string
        let error = system.call_method1(
            "set",
            (
                ("c1a2", "1+c0a1"),
...
/// Test try_set_pauli_product and get_pauli_product functions of FermionLindbladOpenSystem
#[test]
...
/// Test try_set_noise and get_noise functions of FermionLindbladOpenSystem
#[test]
...
/// Test add_pauli_product and get_pauli_product functions of FermionLindbladOpenSystem
#[test]
...
/// Test add_noise and get_noise functions of FermionLindbladOpenSystem
#[test]
...
/// Test copy and deepcopy functions of FermionLindbladOpenSystem
#[test]
...
/// Test to_bincode and from_bincode functions of FermionLindbladOpenSystem
#[test]
...
/// Test to_ and from_json functions of FermionLindbladOpenSystem
#[test]
...
fn test_format_repr() {
    ...
    pyo3::Python::attach(|py| {
        ...
        system = system
            ...
            .unwrap();
        let mut rust_system = FermionLindbladOpenSystem::group(
        ...
        rust_system
            .system_mut()
            .add_operator_product(
                HermitianFermionProduct::new([0], [0]).unwrap(),
...
fn test_richcmp() {
    ...
    pyo3::Python::attach(|py| {
        ...
        assert!(!comparison);
        let comparison =
            bool::extract(system_one.call_method1("__eq__", ("c0a0",)).unwrap().as_borrowed()).unwrap();
        assert!(!comparison);
        ...
        assert!(comparison);
        let comparison =
            bool::extract(system_one.call_method1("__ne__", ("c0a0",)).unwrap().as_borrowed()).unwrap();
        assert!(comparison);
...
/// Test jordan_wigner() method of FermionLindbladOpenSystem
#[test]
fn test_jordan_wigner() {
    ...
    pyo3::Python::attach(|py| {
        ...
        let number_modes =
            usize::extract(flos.call_method0("current_number_modes").unwrap().as_borrowed()).unwrap();
        let number_spins =
            usize::extract(slos.call_method0("current_number_spins").unwrap().as_borrowed()).unwrap();
