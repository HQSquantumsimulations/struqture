...
fn test_jordan_wigner() {
    ...
    pyo3::Python::attach(|py| {
        ...
        let empty = bool::extract(slns.call_method0("is_empty").unwrap().as_borrowed()).unwrap();
        assert!(!empty);

        let number_modes =
            usize::extract(flns.call_method0("current_number_modes").unwrap().as_borrowed()).unwrap();
        let number_spins =
