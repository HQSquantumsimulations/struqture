...
use struqture::{fermions::FermionProduct, STRUQTURE_VERSION};
...
fn new_pp(
    ...
    pp_type
        ...
        .unwrap()
        .downcast::<FermionProductWrapper>()
        .unwrap()
        .to_owned()
}

/// Test default function of FermionProductWrapper
#[test]
fn test_default_partialeq_debug_clone() {
    pyo3::prepare_freethreaded_python();
    Python::with_gil(|py| {
        let pp = new_pp(py, vec![0, 1], vec![1, 2]);
        let pp_wrapper = pp.extract::<FermionProductWrapper>().unwrap();

        // PartialEq
        let helper_ne: bool = FermionProductWrapper::default() != pp_wrapper;
        assert!(helper_ne);
        let helper_eq: bool =
            FermionProductWrapper::default() == FermionProductWrapper::new(vec![], vec![]).unwrap();
        assert!(helper_eq);

        // Test PartialOrd trait
        let pp_0 = new_pp(py, vec![0, 1], vec![1, 2]);
        let pp_wrapper_0 = pp_0.extract::<FermionProductWrapper>().unwrap();
        let pp_1 = new_pp(py, vec![0, 1], vec![1, 3]);
        let pp_wrapper_1 = pp_1.extract::<FermionProductWrapper>().unwrap();
        ...
        assert_eq!(
            format!("{:?}", FermionProductWrapper::new(vec![0, 1], vec![1, 2]).unwrap()),
            "FermionProductWrapper { internal: FermionProduct { creators: [0, 1], annihilators: [1, 2] } }"
        );
    })
}
...
/// Test from_string function of FermionProduct
#[test]
fn test_from_string() {
    pyo3::prepare_freethreaded_python();
    pyo3::Python::with_gil(|py| {
...
/// Test from_string function of FermionProduct - PyValueError
#[test]
...
/// Test creators and annihilators functions of FermionProduct
#[test]
...
/// Test hermitian_conjugate and is_natural_hermitian functions of FermionProduct
#[test]
...
/// Test concatenate functions of FermionProduct
#[test]
fn test_multiply() {
    pyo3::prepare_freethreaded_python();
    pyo3::Python::with_gil(|py| {
        let pp_0 = new_pp(py, vec![84, 95], vec![10, 20]);
        let pp_1 = new_pp(py, vec![10, 30], vec![43, 78]);
        let pp_mul_1 = new_pp(py, vec![10, 30, 84, 95], vec![10, 20, 43, 78]);
        let pp_mul_2 = new_pp(py, vec![30, 84, 95], vec![20, 43, 78]);
...
/// Test copy and deepcopy functions of FermionProduct
#[test]
...
/// Test to_bincode and from_bincode functions of FermionProduct
#[test]
...
/// Test to_ and from_json functions of FermionProduct
#[test]
...
/// Test hash functions of FermionProduct
#[test]
fn test_hash() {
    pyo3::prepare_freethreaded_python();
    pyo3::Python::with_gil(|py| {
...
/// Test jordan_wigner() method of FermionProduct
#[test]
fn test_jordan_wigner() {
    pyo3::prepare_freethreaded_python();
    pyo3::Python::with_gil(|py| {
        let fp = new_pp(py, vec![0, 1], vec![1, 2]);
        let so = fp.call_method0("jordan_wigner").unwrap();

        let empty = bool::extract_bound(&so.call_method0("is_empty").unwrap()).unwrap();
        assert!(!empty);

        let current_number_modes =
            usize::extract_bound(&fp.call_method0("current_number_modes").unwrap()).unwrap();
        let current_number_spins =
            usize::extract_bound(&so.call_method0("current_number_spins").unwrap()).unwrap();
...
fn test_json_schema() {
    ...
    pyo3::Python::with_gil(|py| {
        ...
        let rust_schema =
            serde_json::to_string_pretty(&schemars::schema_for!(FermionProduct)).unwrap();
...
fn test_from_pyany_to_struqture_1() {
    ...
    pyo3::Python::with_gil(|py| {
        ...
        let result = FermionProductWrapper::from_pyany_to_struqture_1(pp_2.as_ref()).unwrap();
        assert_eq!(
            result,
            struqture_1::fermions::FermionProduct::from_str("c0c1a1a2").unwrap()
        );
    });
}
