...
use struqture_py::mixed_systems::MixedOperatorWrapper;
...
/// Test default function of MixedOperatorWrapper
#[test]
fn test_default_partialeq_debug_clone() {
    ...
    Python::with_gil(|py| {
        ...
        new_system
            ...
            .unwrap();
        let system_wrapper = new_system.extract::<MixedOperatorWrapper>().unwrap();

        // PartialEq
        let helper_ne: bool = MixedOperatorWrapper::new(1, 1, 1) != system_wrapper;
        assert!(helper_ne);
        let helper_eq: bool =
            MixedOperatorWrapper::new(1, 1, 1) == MixedOperatorWrapper::new(1, 1, 1);
        assert!(helper_eq);
        ...
        // Debug
        assert_eq!(
            format!("{:?}", MixedOperatorWrapper::new(1, 1, 1)),
            "MixedOperatorWrapper { internal: MixedOperator { internal_map: {}, n_spins: 1, n_bosons: 1, n_fermions: 1 } }"
        );
    })
}

/// Test number_bosons function of MixedOperator
#[test]
...
/// Test hermitian_conjugate function of MixedOperator
#[test]
...
/// Test set and get functions of MixedOperator
#[test]
fn boson_system_test_set_get() {
    pyo3::prepare_freethreaded_python();
    pyo3::Python::with_gil(|py| {
        let new_system = py.get_type_bound::<MixedOperatorWrapper>();
        let number_spins: usize = 1;
        let number_bosons: usize = 1;
        let number_fermions: usize = 1;
        ...
        let binding = new_system
            .call1((number_spins, number_bosons, number_fermions))
            .unwrap();
        let system = binding.downcast::<MixedOperatorWrapper>().unwrap();
        system
        ...
        let error = system.call_method1("set", ("S5X:S7Y:Bc2c3a1:Fc0a2a3:", 0.1));
        assert!(error.is_err());
...
fn boson_system_test_add_operator_product_remove() {
    ...
    pyo3::Python::with_gil(|py| {
        ...
        let system = new_system
            .call1((number_spins, number_bosons, number_fermions))
            .unwrap();
        system.downcast::<MixedOperatorWrapper>().unwrap();
        system
            .call_method1("add_operator_product", ("S0Z:Bc0a1:Fc0a0:", 0.1))
            .unwrap();
        system
        ...
        // Try_set error 3: Number of bosons in entry exceeds number of bosons in system.
        let error = system.call_method1("add_operator_product", ("S5X:S7Y:Bc2c3a1:Fc0a2a3:", 0.1));
        assert!(error.is_err());
...
/// Test keys function of MixedOperator
#[test]
fn test_keys_values() {
    pyo3::prepare_freethreaded_python();
    pyo3::Python::with_gil(|py| {
...
fn test_mul_self() {
    ...
    pyo3::Python::with_gil(|py| {
        ...
        system_0_1
            .call_method1(
                "add_operator_product",
                ("S0Z1X:Bc0c1c2c3a1:Fc0c2a0a2", -0.1),
            )
            .unwrap();
...
/// Test copy and deepcopy functions of MixedOperator
#[test]
...
/// Test to_bincode and from_bincode functions of MixedOperator
#[test]
...
/// Test to_ and from_json functions of MixedOperator
#[test]
...
fn test_format_repr() {
    ...
    pyo3::Python::with_gil(|py| {
        ...
        system
            .call_method1("add_operator_product", ("S0Z:Bc0a1:Fc0a0:", 0.1_f64))
            .unwrap();
        let mut rust_system = MixedOperator::new(1, 1, 1);
        rust_system
            .add_operator_product(
                MixedProduct::new(
...
fn test_json_schema() {
    ...
    pyo3::Python::with_gil(|py| {
        ...
        let rust_schema =
            serde_json::to_string_pretty(&schemars::schema_for!(MixedOperator)).unwrap();
...
fn test_from_pyany_to_struqture_1() {
    ...
    pyo3::Python::with_gil(|py| {
        ...
        let pp_1: struqture_1::mixed_systems::MixedProduct =
            ...
            .unwrap();
        let mut sys_1 = struqture_1::mixed_systems::MixedSystem::new([None], [None], [None, None]);
        struqture_1::OperateOnDensityMatrix::set(&mut sys_1, pp_1.clone(), 0.1.into()).unwrap();

        let result = MixedOperatorWrapper::from_pyany_to_struqture_1(sys_2.as_ref()).unwrap();
...
fn test_from_json_struqture_1() {
    pyo3::prepare_freethreaded_python();
    pyo3::Python::with_gil(|py| {
        let json_string: Bound<pyo3::types::PyString> = pyo3::types::PyString::new_bound(py, "{\"number_spins\":[null],\"number_bosons\":[null],\"number_fermions\":[null],\"operator\":{\"items\":[[\"S0Z:Bc0a0:Fc1a1:\",1.0,0.0]],\"n_spins\":1,\"n_bosons\":1,\"n_fermions\":1,\"_struqture_version\":{\"major_version\":1,\"minor_version\":0}}}");
        ...
        let error_json_string: Bound<pyo3::types::PyString> = pyo3::types::PyString::new_bound(py, "{\"number_spins\":[null],\"number_bosons\":[null],\"number_fermions\":[null],\"operator\":{\"items\":[[\"S0Z:Bc0a0:Fc1a1:\",1.0,0.0]],\"n_spins\":1,\"n_bosons\":1,\"n_fermions\":1,\"_struqture_version\":{\"major_version\":3-,\"minor_version\":0}}}");
