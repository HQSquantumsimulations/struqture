...
fn test_new_errors() {
    pyo3::prepare_freethreaded_python();
    pyo3::Python::with_gil(|py| {
        let pp = py.get_type_bound::<MixedProductWrapper>();
...
fn test_json_schema() {
    ...
    pyo3::Python::with_gil(|py| {
        ...
        let rust_schema =
            serde_json::to_string_pretty(&schemars::schema_for!(MixedProduct)).unwrap();
...
fn test_from_pyany_to_struqture_1() {
    ...
    pyo3::Python::with_gil(|py| {
        ...
        let result = MixedProductWrapper::from_pyany_to_struqture_1(pp_2.as_ref()).unwrap();
