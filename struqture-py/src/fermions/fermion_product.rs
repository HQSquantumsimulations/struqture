...
///
/// The FermionProduct is used as an index for non-hermitian, normal ordered bosonic operators.
/// A bosonic operator can be written as a sum over normal ordered products of creation and annihilation operators.
/// The FermionProduct is used as an index when setting or adding new summands to a bosonic operator and when querrying the
...
/// .. code-block:: python
///
///     from struqture_py.bosons import FermionProduct
///     import numpy.testing as npt
///     # For instance, to represent $c_0a_0$
///     b_product = FermionProduct([0], [0])
///     npt.assert_equal(bp.creators(), [0])
///     npt.assert_equal(bp.annihilators(), [0])
///     
#[pyclass(name = "FermionProduct", module = "struqture_py.bosons")]
